<?php if(!defined('BASEPATH')) exit('No direct script access allowed');
  ...
  */
  class Datatables
  {
    ...
    * @Modified By Farveen Hassan 
    * Purpose For Modification:To Get the Value Into Array While Using Goupby
    * @param string $column
    * @return mixed
    */
  ...
  //Function to generate order by portion of the query
  //Radhu k krishna
  
   /* public function order_by($column,$desc)
    {
      $this->order_bys = $column;
    ...
    * @Author Farveen Hassan
    * @param mixed $key_condition
    * @param string $val
    * @param bool $backtick_protect
    * @return mixed
    */
    public function in_where($key_condition, $val = NULL, $backtick_protect = TRUE)
    {
    ...
    */
     ...
     protected function get_ordering()
    {
      if($this->check_mDataprop()){
        ...
        $mColArray = explode(',', $this->ci->input->post('sColumns'));
        if(!array_filter($mColArray)) {//if scolumns is ,,,,,,,
			  $mColArray =  $this->columns; //then replace it with all coulmsn
		  }
      }else{
        $mColArray = $this->columns;
    ...
    */
    ...
    /**
    * Get result count
    * @modified Farveen Hassan
    * Purpose To Get The Correct count When using Group By - Dated [25-09-1990]
    * @return integer
    */
 ...
 * @return    object
 */
function where_or($key, $value = NULL, $escape = TRUE, $bracket = NULL)
{
