...
class Sendsms
{
	...
	function  intintialze_sms($apiurl,$method,$wk,$sd)
	{
		$this->setMethod($method);
		$this->setWorkingKey($wk);
		$this->setSenderId($sd);
	...
	/**
	 * function to send group sms
	 * 
	 */
	function send_group_sms($to,$message,$dlr_url,$type="xml")
	{
		return $this->process_group_sms($to,$message,$dlr_url,$type="xml",$time="null",$unicode="null");
	}
	...
	function process_group_sms($to,$message,$dlr_url="",$type="xml",$time='',$unicode='')
	{  
		$message=urlencode($message);
		$dlr_url=urlencode($dlr_url);
		$this->to=$to;
	...
	if($unicode=='null')
		...
		$unicode="&unicode=$unicode";
	 	...
	 	$url="$this->start$this->api_url/index.php?method=$this->api_method&api_key=$this->working_key&sender=$this->sender_id&to=$to&message=$message&format=$type&dlr_url=$dlr_url$time$unicode";
	 	//echo $url; // un Comment this to deactivate sms
	 	return  $this->execute($url); // Comment this to deactivate sms
	}
	...
	/**
	 * function to check avaliable credits
	 *  
	 */
	function sms_credits()
	{
		 $url="$this->start$this->api_url/index.php?method=account.credits&api_key=$this->working_key&format=xml";
		 return $this->execute($url);
